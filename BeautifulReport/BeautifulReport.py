...
from Common.CommonTools.PathTools import htmlreport_path,root_path
...
class OutputRedirector(object):
    """ Wrapper to redirect stdout or stderr:封装重新定义标准输出和错误输出 """
...
stdout_redirector = OutputRedirector(sys.stdout)       #定义标准输出到fp文件
stderr_redirector = OutputRedirector(sys.stderr)       #定义错误输出到fp文件

SYSSTR = platform.system()                             #获取系统名称
SITE_PAKAGE_PATH = root_path                           #定义所在的根目录
...
class ReportTestResult(unittest.TestResult):
    ...
    def success_counter(self, value) -> None:
        ...
        :return:
        """
        self.success_count = value      #获取成功case数量
    ...
    def stopTest(self, test) -> None:
        ...
        :return:
        """
        self.end_time = '{0:.3} s'.format((time.time() - self.start_time))      #计算运行时间
    ...
    def stopTestRun(self, title=None) -> dict:
        ...
        FIELDS['testError'] = self.error_count              #失败用例数
...
class BeautifulReport(ReportTestResult, PATH):          #分别继承ReportTestResult、PATH类
    ...
    def report(self, description, filename: str = None, log_path=htmlreport_path):
        """
            生成测试报告,并放在当前运行路径下
        ...
        :return:
        """
        if filename:    #判断filename是否有后缀.html,没有则添加后缀名并赋值
            self.filename = filename if filename.endswith('.html') else filename + '.html'
        
        if description:
            self.title = description
        
        self.log_path = os.path.abspath(log_path)       #获取log_path的绝对值
